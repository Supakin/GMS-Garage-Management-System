...
$check = "SELECT COUNT(EMP_ID) FROM SCHEDULE WHERE SCH_DATE=CURDATE() AND EMP_ID='$id'";
$check_query = mysql_query($check) or die (mysql_error());

if($check_query !=null){
$sql = "INSERT INTO SCHEDULE
  (SCH_ID,EMP_ID,SCH_DATE,SCH_STARTTIME)
  VALUES (
      CONCAT(
...
if($_POST["emp_id"]!=null && strlen($_POST["emp_id"])==6){
  if($sql_query){
    ...
    echo "<script type='text/javascript'>alert('แย่แล้ว!!');window.history.go(-1);</script>" ;
  }
}else{
