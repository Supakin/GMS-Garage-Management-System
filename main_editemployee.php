    ...
    <div class="row justify-content-center">
      <div class="col-5">
          ...
          <div class="form-group">
            ...
            <input name="emp_fname" type="text" class="form-control" id="emp_fname" value="<?=$row['EMP_FNAME'] ?>">
          </div>
          <div class="form-group">
            <label for="emp_lname">นามสกุล :</label>
            <input name="emp_lname" type="text" class="form-control" id="emp_lname" value="<?=$row['EMP_LNAME'] ?>">
          </div>
          <div class="form-group">
            <label for="emp_tel">เบอร์เบอร์โทรติดต่อ :</label>
            <input name="emp_tel" type="text" class="form-control" id="emp_tel" value="<?=$row['EMP_TEL'] ?>">
          </div>
          <div clas="form-group">
            <label for="emp_address">ที่อยู่ :</label>
            <textarea name="emp_address" class="form-control" rows="2" id="emp_address" ><?=$row['EMP_ADDRESS'] ?></textarea>
          </div>
          <div class="form-group">
            <label for="emp_salary">เงินเดือน :</label>
            <input name="emp_salary" type="text" class="form-control" id="emp_salary" value="<?=$row['EMP_SALARY'] ?>">
          </div>
          <div class="form-group">
            <label for="emp_status">สถานะ :</label>
            <div class ="row">
              <div class="col-4 ">
                <input type="radio" name="emp_status"  value="Y" <?php if($row['EMP_STATUS'] == 'Y') echo "checked"; ?>> ทำงาน
              </div>
              <div class="col-4">
                <input type="radio" name="emp_status"  value="N" <?php if($row['EMP_STATUS'] == 'N') echo "checked"; ?>> ออกแล้ว<br>
              </div>
            </div>
          </div>
      </div>
